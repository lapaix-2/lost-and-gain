...
function Settings() {
  ...
  const fetchSettings = async () => {
    try {
      const res = await axios.get('https://lost-and-gain-backend.onrender.com/api/auth/settings/' + user.id);
      setSettings(res.data);
      setTheme(res.data.theme || 'light');
      setLanguage(res.data.language || 'english');
      if (res.data.profile_pic) {
        setPreviewPic('https://lost-and-gain-backend.onrender.com/uploads/' + res.data.profile_pic);
      }
      setLoading(false);
    } catch (err) {
      console.log(err);
  ...
  const handleSaveTheme = async () => {
    try {
      await axios.put('https://lost-and-gain-backend.onrender.com/api/auth/settings/theme/' + user.id, { theme });
  ...
  const handleSaveLanguage = async () => {
    try {
      await axios.put('https://lost-and-gain-backend.onrender.com/api/auth/settings/language/' + user.id, { language });
  ...
  const handleUploadPic = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        'https://lost-and-gain-backend.onrender.com/api/auth/settings/profilepic/' + user.id,
        formData
      );
      setPreviewPic('https://lost-and-gain-backend.onrender.com/uploads/' + res.data.filename);
      setMessage(t.saved);
      setTimeout(() => setMessage(''), 3000);
    } catch (err) { setMessage(t.error); }
  };
  ...
  const handleRemovePic = async () => {
    try {
      await axios.delete('https://lost-and-gain-backend.onrender.com/api/auth/settings/profilepic/' + user.id);
      setPreviewPic(null);
      setProfilePic(null);
      setMessage(t.saved);
      setTimeout(() => setMessage(''), 3000);
  ...
  const handleDeleteAccount = async () => {
    ...
    if (confirmation) {
      try {
        const response = await axios.delete('https://lost-and-gain-backend.onrender.com/api/auth/delete-account', {
